...
function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    console.log('Signup body:', body); // Added for debugging
  ...
  useEffect(() => {
    const fetchCountries = async () => {
      try {
        ...
        console.log("Countries fetched:", response.data.data); // Added for debugging
        setCountries(response.data.data);
      } catch (error) {
        console.error('Error fetching countries:', error);
      }
  ...
  useEffect(() => {
    const fetchStates = async () => {
      if (registerValues.countryId) {
        try {
          const response = await axios.get(
            role === 'student' ? `https://novajobs.us/api/students/states/${registerValues.countryId}` : `https://novajobs.us/api/trainers/states/${registerValues.countryId}`
          );
          console.log("States fetched:", response.data.data); // Added for debugging
          setStates(response.data.data);
        } catch (error) {
          console.error('Error fetching states:', error);
        }
    ...
    const fetchCities = async () => {
      if (registerValues.stateId) {
        try {
          ...
          console.log("Cities fetched:", response.data.data); // Added for debugging
          setCities(response.data.data);
        } catch (error) {
          console.error('Error fetching cities:', error);
        }
  ...
  useEffect(() => {
    if (role === 'student') {
      const fetchQualifications = async () => {
        try {
          const response = await axios.get('https://novajobs.us/api/students/qualifications');
          console.log("Qualifications fetched:", response.data.data); // Added for debugging
          setQualifications(response.data.data);
        } catch (error) {
          console.error('Error fetching qualifications:', error);
        }
